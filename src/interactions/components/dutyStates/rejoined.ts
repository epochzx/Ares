...
const event: Component = {
    ...
    execute: async (interaction: ButtonInteraction) => {
        ...
        const newNote = 
            oldNote === 'N/A' ? `Rejoined ${timeNow}` :
            oldNote.startsWith('Disconnected') ? `${oldNote}, rejoined ${timeNow}` :
            `${oldNote}. Rejoined ${timeNow}`;
        ...
        await reply(true, `Successfully noted your rejoin.`, interaction, undefined, false, true);
        await logUserInteraction(interaction.member as GuildMember, `noted a rejoin`);
    }   
};
